...
namespace Lab._5._2._26_27_28
{
    internal class Program
    {
        ...
        static void mon(double[,] x, string str)
        {
            ...
            if (s[0] == "не")
            {
                Console.WriteLine(" Промежутки монотонности y1:");
                n = 0;
                while (n < x.GetLength(1) - 1)
                {
                    ...
                    {
                        m = 0;
                        while (n < x.GetLength(1) - 1 && x[1, n] >= x[1, n + 1])
                        {
                            n = n + 1;
                            m = m + 1;
                        }
            ...
            if (s[4] == "не")
            {
                Console.WriteLine(" Промежутки монотонности y2:");
                n = 0;
                while (n < x.GetLength(1) - 1)
                {
                    if (x[2, n] >= x[2, n + 1])
                    {
                        m = 0;
                        while (n < x.GetLength(1) - 1 && x[2, n] >= x[2, n + 1])
                        {
                            n = n + 1;
                            m = m + 1;
                        }
        ...
        static void Main(string[] args)
        {
            ...
            double[,] tab = { { 1, 3, 3, 4, 5 }, { 1, -3, 3, -1, -9 }, { 9, 9, 7, 7, -1, } };
            //generate(tab, A, B);
            ...
            Console.Write(" а) Функция y1 и y2 " + str+ " на отрезке [A, B] соотвественно\n б)");
            mon(tab, str);
